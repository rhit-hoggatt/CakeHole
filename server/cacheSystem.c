...
int get_from_cache_safe(const char *domain, char *ip_buf, size_t buf_size) {
  if (!domain || !ip_buf || buf_size == 0)
    return -1;
  pthread_mutex_lock(&cache_mutex);
  IPUrlPair *pair = find(cache_list, domain);
  if (pair != NULL) {
    snprintf(ip_buf, buf_size, "%s", pair->ip);
    pthread_mutex_unlock(&cache_mutex);
    return 0;
  }
  pthread_mutex_unlock(&cache_mutex);
...
int get_from_adcache_safe(const char *domain, char *ip_buf, size_t buf_size) {
  if (!domain || !ip_buf || buf_size == 0)
    return -1;
  pthread_mutex_lock(&adlist_mutex);
  IPUrlPair *pair = find(adlist, domain);
...
int add_addlists() {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    while (fgets(line, sizeof(line), file)) {
      ...
      if (tokensRead == 1) {
        // If only one token is read, treat it as a domain and assign IP
        // "0.0.0.0"
