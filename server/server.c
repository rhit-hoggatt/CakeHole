...
int main(int argc, char *argv[]) {
  ...
  int log_fd = open("adlists/metadata/server_logs.txt",
  ...
  // Initialize DHCP server (loads config, starts if enabled)
  if (dhcp_server_init() != 0) {
    fprintf(stderr, "Warning: Failed to initialize DHCP server\n");
  } else if (dhcp_is_enabled()) {
    if (dhcp_server_start() != 0) {
      fprintf(stderr, "Warning: Failed to start DHCP server\n");
    }
  }
  int sockfd;
  ...
  for (int i = 0; i < THREAD_COUNT; i++) {
    thread_numbers[i] = i;
    if (pthread_create(&threads[i], NULL, processDNS, &thread_numbers[i]) !=
        0) {
